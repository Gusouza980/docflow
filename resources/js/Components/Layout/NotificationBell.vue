...
const unreadCount = ref(page.props.auth?.notifications?.unread_count ?? 0);

const hasUnread = computed(() => unreadCount.value > 0);

watch(() => page.props.auth?.notifications?.unread_count, (count) => {
...
<template>
    <div class="relative">
        ...
        <div
            v-if="open"
            class="absolute right-0 z-30 mt-2 w-[22rem] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl"
        >
            ...
            <ul v-else class="max-h-96 overflow-y-auto">
                <li v-for="notification in notifications" :key="notification.id" class="border-b border-slate-100 last:border-0">
                    <button
                        type="button"
